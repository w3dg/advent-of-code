...
groupInputs.forEach((groupInput) => {
  ...
  let commonAnswers = simplifiedGroupInput.reduce((acc, curr) => {
    // we filter out the answers that are common
    return acc.filter((answer) => curr.includes(answer));
  }, simplifiedGroupInput[0].split(""));

  // console.log(commonAnswers);

  sumOfCounts += commonAnswers.length;
});
